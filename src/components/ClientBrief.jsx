...
import { 
  Building2, AlertTriangle, Sparkles, FileText, Users2, 
  TrendingUp, Users, Handshake, AlertCircle, CheckCircle2, 
  Zap, Lightbulb, Calendar, Minus
} from "lucide-react";
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
      {list.map((item, i) => (
        <div key={i} style={{ display: "flex", gap: 6, alignItems: "flex-start" }}>
          <span style={{ color: color || C.accent, fontSize: 9, marginTop: 4, flexShrink: 0 }}>
...
function Blk({ num, icon: Icon, title, accent, children }) {
  return (
    <div style={{ background: C.card, border: `1px solid ${accent ? C.accentBd : C.border}`, borderLeft: accent ? `3px solid ${C.accent}` : undefined, borderRadius: 12, padding: "13px 15px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 9 }}>
        <Icon size={12} color={accent ? C.accent : C.muted} />
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <Blk num="1" icon={Building2} title="Contexto del Cliente">
            <Kv k="Situación" v={b.seccion_1_contexto?.situacion_actual} />
          </Blk>
          <Blk num="2" icon={AlertTriangle} title="Pain Points">
          ...
          <Blk num="3" icon={Sparkles} title="Qué Valoró de WorldTeams">
            <div style={{ marginBottom: 7 }}>
          ...
          <Blk num="4" icon={FileText} title="Servicio Contratado" accent>
          ...
          <Blk num="7" icon={Users2} title="Dinámica del Cliente">
            ...
            {b.seccion_7_dinamica_cliente?.quien_bloquea && (
              <div style={{ display: "flex", gap: 6, marginTop: 4, alignItems: "flex-start" }}>
                <AlertTriangle size={12} color={C.amber} style={{ marginTop: 2 }} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <Blk num="5" icon={TrendingUp} title="Oportunidades de Expansión">
          ...
          <Blk num="6" icon={Users} title="Posiciones Mencionadas">
            <BList items={b.seccion_6_posiciones_mencionadas?.map(p => `${p.rol}: ${p.contexto}`)} />
          </Blk>
          <Blk num="8" icon={Handshake} title="Información Relacional">
          ...
          <Blk num="9" icon={b.seccion_9_riesgos_alertas?.length ? AlertCircle : CheckCircle2} title="Riesgos / Alertas">
          ...
          <Blk num="10" icon={Zap} title="Momentos Clave">
            <BList items={b.seccion_10_momentos_clave} />
          </Blk>
          <Blk num="11" icon={Lightbulb} title="Recomendaciones para Activation" accent>
          ...
          <Blk num="12" icon={Calendar} title="Próximos Pasos">
